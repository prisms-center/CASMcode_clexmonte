...
/// \brief Constructor
SystemSupercellData::SystemSupercellData(
    ClexData const &_formation_energy_clex_data,
    ...
    : supercell_neighbor_list(std::make_shared<clexulator::SuperNeighborList>(
      ...
      formation_energy_clex(supercell_neighbor_list, clex_data.clexulator,
                            clex_data.eci) {}

/// \brief Helper to get CanonicalSupercellData, constructing as necessary
///
/// \relates OccSystem
SystemSupercellData &get_supercell_data(
  ...
  if (it == data.supercell_data.end()) {
    it = data.supercell_data
             .emplace(std::piecewise_construct std::forward_as_tuple(
                          transformation_matrix_to_super),
                      std::forward_as_tuple(data.clex_data,
                                            transformation_matrix_to_super))
             .first;
  }
  return it->second;
...
Configuration from_standard_values(
    OccSystem const &data,
    Eigen::Matrix3l const &configuration_in_standard_basis) {
  return Configuration(
      ...
      clexulator::from_standard_values(
          ...
          data.shared_prim->basis().size(),
          transformation_matrix_to_super.determinant(), data.global_dof_info,
          data.local_dof_info));
}

/// \brief Convert configuration from prim basis to standard basis
Configuration to_standard_values(
    OccSystem const &data, Eigen::Matrix3l const &configuration_in_prim_basis) {
  return Configuration(

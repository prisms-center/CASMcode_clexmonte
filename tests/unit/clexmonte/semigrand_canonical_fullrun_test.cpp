...
TEST(semigrand_canonical_fullrun_test, Test1) {
  ...
  fs::path test_dir = fs::current_path() / "CASM_test_projects" /
                      "semigrand_canonical_fullrun_test";
  ...
  std::runtime_error error_if_invalid{
      "Error reading semi-grand canonical Monte Carlo JSON input"};
  ...
  // ### Construct the semi-grand canonical calculator
  ...
  EXPECT_EQ(sampling_functions.size(), 7);
  ...
  EXPECT_EQ(sampling_functions.count("param_chem_pot"), 1);
  ...
  monte::ValueMap initial_conditions =
      clexmonte::semigrand_canonical::make_conditions(
          1000.0,                 // temperature (K)
          composition_converter,  // composition converter
          {{"a", -4.0}});         // param_chem_pot values

  // - Construct conditions increment
  monte::ValueMap conditions_increment =
      clexmonte::semigrand_canonical::make_conditions_increment(
          0.0,                    // temperature (K)
          composition_converter,  // composition converter
          {{"a", 0.4}});          // param_chem_pot values
